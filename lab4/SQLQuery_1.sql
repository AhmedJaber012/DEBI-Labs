...
FROM '/Employee_table.csv'  -- Ensure this matches the exact Linux path inside the container
WITH
(                           -- Use Parentheses here
    FORMAT = 'CSV',         -- Use 'CSV'
    FIRSTROW = 2,
    FIELDTERMINATOR = ',',  -- Optional: Needed if your file uses commas
    ROWTERMINATOR = '\n'    -- Optional: Needed for Linux-style line breaks
);


-- Step 2: Insert the data by manual way
...
-- ----------------
SELECT EMPNO, FIRSTNME, WORKDEPT, SALARY, HIREDATE,
AVG(SALARY) OVER(PARTITION BY WORKDEPT ORDER BY HIREDATE) AVG
FROM employee;
-- -----------------
